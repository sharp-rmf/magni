...
if [ $? != 0 ]; then
    ...
    # roscore
    tmux send-keys -t tmux_run 'ros1_melodic; source ~/devop_ws/devel/setup.bash; roscore' C-m
    ...
    tmux send-keys -t tmux_run 'sleep 14; ros1_melodic; source ~/devop_ws/devel/setup.bash;roslaunch sliding_autonomy start_mobile.launch robotname:=magni' C-m
    tmux select-layout tiled

    # intelligence
    tmux split-window -h -t tmux_run
    tmux send-keys -t tmux_run 'sleep 16; ros1_melodic; source ~/devop_ws/devel/setup.bash;roslaunch sliding_autonomy start_intelligence_no_rbpearl.launch robotname:=magni' C-m
    tmux select-layout tiled

    # merge sick lasers
    tmux split-window -h -t tmux_run
    tmux send-keys -t tmux_run 'sleep 18; ros1_melodic; source ~/devop_ws/devel/setup.bash;roslaunch /opt/ros/melodic/share/sliding_autonomy/launch/magni/merge_lidar_scan_no_rbpearl.launch' C-m
    tmux select-layout tiled

    # navigation
    tmux split-window -h -t tmux_run
    # tmux send-keys -t tmux_run 'sleep 14; ros1_melodic; roslaunch sliding_autonomy start_navigation_magni.launch yaml_map:=cafe_level_1 robotname:=magni ' C-m
    tmux send-keys -t tmux_run 'sleep 14; ros1_melodic; source ~/devop_ws/devel/setup.bash;roslaunch sliding_autonomy start_navigation_magni_no_rbpearl.launch yaml_map:=chart_map robotname:=magni gui:=true ' C-m
    tmux select-layout tiled

    # line following
    tmux split-window -h -t tmux_run
    tmux send-keys -t tmux_run 'sleep 14; ros1_melodic; source ~/devop_ws/devel/setup.bash;roslaunch task_handler_line_following start.launch robotname:=magni' C-m
    tmux select-layout tiled

    # line following
    # tmux split-window -h -t tmux_run
    # tmux send-keys -t tmux_run 'sleep 12; ros1_melodic; roslaunch task_handler_pursuit start.launch robotname:=sml' C-m
    # tmux select-layout tiled
